...
import { Star, Heart, Eye, ShoppingBag, ChevronRight, Sparkles, SlidersHorizontal, PackageX } from "lucide-react";
import { useShop } from "@/context/ShopContext";
import { isProductOutOfStock } from "@/lib/productAdapter";

const defaultNewArrivalProducts = [
  {
    id: "na-1",
...
export function SkinCareSection({ products: initialProducts }: NewArrivalsSectionProps) {
  const { addToCart, addToWishlist, isInWishlist, setQuickViewProduct } = useShop();
  const [items, setItems] = useState<any[]>(defaultNewArrivalProducts);
    ...
    const processProducts = (rawList: any[]) => {
      ...
      const normalized = rawList.map((p: any) => {
        ...
        return {
          ...
          isSale: Boolean(p.isSale || p.sale_price || origPrice),
      ...
      // Prioritize new arrivals first
      const newItems = normalized.filter((p) => p.isNew);
      const otherItems = normalized.filter((p) => !p.isNew);
  ...
  return (
    <div className="w-full max-w-[1680px] mx-auto px-4 sm:px-8 py-6">
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-0 border border-slate-200/80 rounded-2xl overflow-hidden bg-white shadow-xs">
        ...
        <div className="lg:col-span-4 bg-gradient-to-b from-[#FF6600] to-[#E65A00] p-6 sm:p-8 lg:p-10 flex flex-col justify-center relative overflow-hidden">
          ...
          <div className="relative z-10 space-y-4">
            <div className="inline-flex items-center gap-1.5 px-3 py-1 bg-white/20 backdrop-blur-md rounded-full text-white text-[11px] font-bold uppercase tracking-wider">
            ...
            <p className="text-white/85 text-xs sm:text-sm font-medium leading-relaxed max-w-xs">
              Explore the latest premium additions to SMT Mart BD with exclusive launch offers.
            </p>

            <div className="pt-2">
              <Link
                href="/new-arrivals"
                className="inline-flex items-center gap-1.5 px-6 py-2.5 bg-white hover:bg-[#002B49] text-[#FF6600] hover:text-white font-black text-xs sm:text-sm rounded-full shadow-md transition-all duration-300 group/btn cursor-pointer active:scale-95"
              >
                <span>See More</span>
        ...
        {/* Right Side: Responsive Product Cards Grid (Col 8, 2-cols mobile) */}
        <div className="lg:col-span-8 bg-slate-200 grid grid-cols-2 sm:grid-cols-2 md:grid-cols-3 gap-px">
          {items.map((prod, idx) => {
            ...
            return (
              <div
                key={`${prod.id}-${idx}`}
                className="p-2.5 sm:p-4 flex flex-col justify-between bg-white hover:shadow-lg transition-all duration-300 relative group/card overflow-hidden"
              >
                ...
                {/* Product Top: Badge & Image */}
                <div>
                  <div className={`w-full h-32 sm:h-44 bg-white rounded-lg flex items-center justify-center overflow-hidden relative mb-2 sm:mb-3 group-hover/card:scale-105 transition-transform duration-300 ${isOutOfStock ? "opacity-75" : ""}`}>
                    {/* Badges (Out of Stock / Sale / New) */}
                    <div className="absolute top-0 left-0 z-10 flex flex-col gap-1 pointer-events-none">
                      {isOutOfStock ? (
                        ...
                        <>
                          {prod.isNew && (
                            <span className="bg-emerald-600 text-white text-[9px] sm:text-[10px] font-black px-1.5 sm:px-2 py-0.5 rounded-full uppercase tracking-wider shadow-xs">
                              NEW
                            </span>
                          )}
                          {Boolean(prod.discountPercentage && prod.discountPercentage > 0) && (
                            <span className="bg-[#FF6600] text-white text-[9px] sm:text-[10px] font-black px-1.5 sm:px-2 py-0.5 rounded-full uppercase tracking-wider shadow-xs">
                    ...
                    <Link href={`/product/${prod.slug || prod.id}`} className="relative w-full h-full block">
                      <img
                        ...
                        onError={(e) => {
                          (e.target as HTMLImageElement).src = "/prod_serum.png";
                        }}
                      />
                    </Link>
                  </div>
                  ...
                  <Link href={`/product/${prod.slug || prod.id}`}>
                    <h3 className="font-semibold text-slate-800 text-[11px] sm:text-xs line-clamp-2 hover:text-[#FF6600] transition leading-tight sm:leading-snug min-h-[28px] sm:min-h-[32px] mt-0.5 mb-1.5">
                      {prod.name}
                    </h3>
                  </Link>
                </div>
...
export const NewArrivalsSection = SkinCareSection;
