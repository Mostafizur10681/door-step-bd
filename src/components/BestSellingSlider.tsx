...
import { ChevronLeft, ChevronRight, Star, Heart, Eye, ShoppingBag, SlidersHorizontal, PackageX } from "lucide-react";
...
export function BestSellingSlider({ products }: { products: any[] }) {
  ...
  const normalizedList = (products || []).map((p) => {
    ...
    return {
      ...
      mainImage: p.main_image || p.image || p.mainImage || (Array.isArray(p.images) && p.images[0]) || "https://placehold.co/300x300/f1f5f9/94a3b8?text=Product",
      rating: typeof p.rating === "number" ? p.rating : (parseFloat(String(p.rating || 5)) || 5),
  ...
  const otherProducts = normalizedList.filter((p) => !p.isBestSeller);
  const displayList = bestSellers.length > 0 ? [...bestSellers, ...otherProducts] : normalizedList;
  ...
  return (
    <div
      className="w-full space-y-4 pt-6 sm:pt-8 pb-3 relative z-10"
      onMouseEnter={() => setIsPaused(true)}
      onMouseLeave={() => setIsPaused(false)}
    >
      {/* Section Header: Centered Title */}
      <div className="max-w-[1500px] mx-auto px-4 sm:px-8 flex items-center justify-center mb-3 sm:mb-4">
        <h2 className="text-2xl sm:text-3xl font-extrabold text-[#122B5A] tracking-wide text-center">
          Best Selling Products
        </h2>
      </div>

      {/* Slider Container (Expanded Width max-w-[1500px]) */}
      <div className="relative max-w-[1500px] mx-auto px-6 sm:px-8 md:px-10">
        {/* Navigation Buttons */}
        <button
          onClick={handlePrev}
          type="button"
          aria-label="Previous Best Selling Products"
          className="absolute left-1 sm:left-2 md:left-3 top-1/2 -translate-y-1/2 bg-white text-slate-700 hover:text-[#122B5A] hover:border-[#FFB800] p-2.5 sm:p-3 rounded-full shadow-xl border border-slate-200 z-30 transition-all hover:scale-105 active:scale-95 cursor-pointer"
        >
        ...
        <button
          ...
          className="absolute right-1 sm:right-2 md:right-3 top-1/2 -translate-y-1/2 bg-white text-slate-700 hover:text-[#122B5A] hover:border-[#FFB800] p-2.5 sm:p-3 rounded-full shadow-xl border border-slate-200 z-30 transition-all hover:scale-105 active:scale-95 cursor-pointer"
        >
          <ChevronRight className="w-5 h-5 sm:w-6 sm:h-6 stroke-[3]" />
        </button>

        {/* Responsive Grid Card Row (2 columns on mobile, 5 on desktop) */}
        <div className="grid grid-cols-2 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-px bg-slate-200 border border-slate-200 rounded-xl overflow-hidden shadow-xs relative z-10">
          {visibleProducts.map((prod, idx) => {
            ...
            const hasVariants = Boolean(
              ...
              (typeof p.attributes === "string" && p.attributes.length > 2 && p.attributes !== "[]" && p.attributes !== "{}") ||
              p.has_variants ||
              p.hasVariants ||
            ...
            return (
              <div
                key={`${prod.id}-${idx}`}
                className="p-2.5 sm:p-4 flex flex-col justify-between hover:shadow-xl transition-all duration-300 bg-white relative group/card overflow-hidden"
              >
                {/* OUT OF STOCK or SALE / DISCOUNT Badge */}
                {isOutOfStock ? (
                  <span className="absolute top-2 left-2 z-10 bg-rose-600 text-white text-[9px] sm:text-[10px] font-black px-1.5 sm:px-2 py-0.5 rounded-full uppercase tracking-wider shadow-xs flex items-center gap-1 pointer-events-none">
                    <PackageX className="w-3 h-3" /> OUT OF STOCK
                  </span>
                ) : Boolean(prod.discountPercentage && prod.discountPercentage > 0) ? (
                  <span className="absolute top-2 left-2 z-10 bg-[#FFB800] text-[#122B5A] font-bold text-[9px] sm:text-[10px] font-black px-1.5 sm:px-2 py-0.5 rounded-full uppercase tracking-wider shadow-xs">
                ...
                <div>
                  {/* Image Container */}
                  <div className={`w-full h-32 sm:h-44 bg-white rounded-md flex items-center justify-center overflow-hidden relative mb-2 sm:mb-3 group-hover/card:scale-105 transition-transform duration-300 ${isOutOfStock ? "opacity-75" : ""}`}>
                ...
                <div className="mt-1 sm:mt-2 space-y-2 pt-1.5 sm:pt-2 border-t border-slate-100">
                  <div className="flex items-center justify-between gap-1 flex-wrap">
                    ...
                    {/* Star rating icons */}
                    <div className="flex items-center text-slate-300 gap-0.5 text-[9px] sm:text-[10px]">
                      {[...Array(5)].map((_, i) => (
                        <Star
                          key={i}
                  ...
                  {/* Mobile & Desktop Action Button (Out of Stock vs Variant vs Direct Add) */}
                  {isOutOfStock ? (
                    <button
                      type="button"
                      disabled
      ...
      <div className="flex items-center justify-center pt-6">
        <Link
          href="/bestsellers"
          className="inline-flex items-center gap-2 px-8 py-2.5 bg-[#122B5A] hover:bg-[#0B1B38] text-white font-bold rounded-full font-bold text-xs sm:text-sm transition-all duration-300 shadow-md hover:shadow-amber-500/25 hover:scale-105 active:scale-95 group/see"
        >
          <span>See More</span>
