...
export function LatestProductsSlider({ products }: { products: any[] }) {
  ...
  const normalizedList = (products || []).map((p) => {
    ...
    return {
      ...
      mainImage: p.main_image || p.image || p.mainImage || (Array.isArray(p.images) && p.images[0]) || "/prod_honey.png",
  ...
  const displayList = normalizedList.length > 0 ? normalizedList : [];
  const totalItems = displayList.length;
  const totalPages = Math.max(1, totalItems - visibleCount + 1);
  ...
  return (
    <div
      className="w-full space-y-4 pt-1 sm:pt-2 pb-5 relative z-10"
