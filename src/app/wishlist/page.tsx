...
import { X, Check, Heart, ArrowRight, PackageX } from "lucide-react";
  ...
  return (
    <div className="bg-white min-h-screen font-sans py-10 pb-20">
      <div className="max-w-[1500px] mx-auto px-4 space-y-8">
        ...
        {/* Title */}
        <h1 className="text-3xl font-extrabold text-[#122B5A] tracking-tight">
          My wishlist
        </h1>
        ...
        {wishlist.length === 0 ? (
          ...
          <div className="bg-slate-50 rounded-2xl border border-slate-200 p-12 text-center max-w-md mx-auto space-y-4">
            ...
            <Link
              href="/"
              className="inline-flex items-center gap-2 bg-[#122B5A] hover:bg-[#0B1B38] text-white font-bold text-xs px-6 py-3 rounded-full transition"
            >
          ...
          <div className="space-y-6">
            <div className="border border-slate-200 rounded-xl overflow-hidden shadow-xs">
              <table className="w-full text-left border-collapse">
                ...
                {/* Table Header */}
                <thead className="bg-[#f2f5f8] border-b border-slate-200 text-[#122B5A] font-bold text-xs">
                  <tr>
                    <th className="py-3.5 px-4 w-12 text-center"></th>
                    <th className="py-3.5 px-4 w-24"></th>
                    <th className="py-3.5 px-6">Product name</th>
                    <th className="py-3.5 px-6 text-[#122B5A]">Unit price</th>
                    <th className="py-3.5 px-6 text-[#122B5A]">Stock status</th>
                  ...
                  {wishlist.map((item) => (
                    <tr key={item.id} className="hover:bg-slate-50/60 transition group">
                      ...
                      <td className="py-4 px-6 font-semibold text-slate-800">
                        <Link href={`/product/${item.slug || item.id}`} className="hover:text-[#122B5A] transition">
                          {item.name}
                        </Link>
                      </td>
                      ...
                      <td className="py-4 px-6">
                        <div className="space-y-0.5">
                          ...
                          <div className="text-[#122B5A] font-bold text-sm">
                      ...
                      <td className="py-4 px-6">
                        {isProductOutOfStock(item) ? (
                          <span className="inline-flex items-center gap-1 font-bold text-rose-600 text-xs">
                            <PackageX className="w-4 h-4 text-rose-600" /> Out of Stock
                          </span>
                        ) : (
                      ...
                      <td className="py-4 px-6 text-right">
                        {isProductOutOfStock(item) ? (
                          <button
                            type="button"
                            disabled
                            className="border border-slate-200 text-slate-400 bg-slate-100 font-bold text-xs py-2 px-4 rounded-full cursor-not-allowed inline-flex items-center gap-1.5"
                          >
                          ...
                          <button
                            ...
                            className="border border-[#122B5A] text-[#122B5A] hover:bg-[#122B5A] hover:text-white font-bold text-xs py-2 px-5 rounded-full transition-all duration-200 cursor-pointer"
                          >
                            Add to cart
                          </button>
                        )}
