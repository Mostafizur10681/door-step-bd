...
function TrackOrderContent() {
  ...
  const initialOrderNo = searchParams.get("order_number") || "";
  ...
  useEffect(() => {
    fetch(`${API_V1}/order-statuses/active`)
      .then((res) => res.json())
      .then((data) => {
  ...
  const fetchTracking = async (numberToTrack: string) => {
    ...
    try {
      const res = await fetch(`${API_V1}/orders/track/${encodeURIComponent(cleanNo)}`);
      ...
      if (res.ok && json.data) {
        ...
        const itemsList: TrackedOrderItem[] = Array.isArray(o.items) && o.items.length > 0
          ? o.items.map((it: any) => {
            ...
            if (it.attributes) {
              if (typeof it.attributes === "string") {
                try { itemAttrs = JSON.parse(it.attributes); } catch { itemAttrs = null; }
              } else if (typeof it.attributes === "object" && !Array.isArray(it.attributes)) {
                itemAttrs = it.attributes;
              }
            }
            return {
              id: it.id,
        ...
        // Format full address (handling ship_different if selected)
        const isShipDiff = Boolean(o.ship_different) || Boolean(o.ship_address && o.ship_address !== o.address);
        let fullAddress = "";
        if (isShipDiff && o.ship_address) {
          fullAddress = [
            o.ship_address,
            o.ship_town_city,
            o.ship_district,
        ...
        const recipientName = isShipDiff && o.ship_customer_name ? o.ship_customer_name : (o.customer_name || "Valued Customer");
        const recipientPhone = isShipDiff && o.ship_phone ? o.ship_phone : (o.customer_phone || "N/A");
        ...
        setTrackedOrder({
          ...
          shippingAmount: parseFloat(String(o.shipping_amount || 80)),
  ...
  return (
    <div className="bg-slate-50 min-h-screen font-sans pb-20 space-y-10">
      ...
      {/* Hero Banner */}
      <section className="bg-gradient-to-r from-[#002B49] via-[#001C30] to-[#FF6600] text-white py-12 sm:py-16 relative overflow-hidden">
            ...
            <div className="flex flex-col sm:flex-row gap-3">
              <div className="relative flex-1">
                <input
                  ...
                  className="w-full bg-slate-50 border border-slate-200 rounded-2xl px-4 py-3.5 text-xs sm:text-sm font-mono text-slate-900 focus:outline-none focus:bg-white focus:ring-2 focus:ring-[#002B49]/30 transition"
                />
              </div>
              ...
              <button
                type="submit"
                disabled={loading}
                className="bg-[#002B49] hover:bg-[#FF6600] text-white font-bold text-xs sm:text-sm py-3.5 px-8 rounded-2xl shadow-md hover:shadow-lg transition-all duration-300 flex items-center justify-center gap-2 shrink-0 cursor-pointer disabled:opacity-60"
              >
        ...
        {trackedOrder && !loading && (
          <div className="bg-white rounded-3xl border border-slate-200/80 p-6 sm:p-8 shadow-sm space-y-8 animate-in fade-in duration-300">
            ...
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-slate-100 pb-6">
              <div className="space-y-1">
                <div className="flex items-center gap-2">
                  ...
                  <span className="font-mono font-extrabold text-[#002B49] text-sm">
              ...
              <div>
                {isCancelled ? (
                  ...
                  <span className="inline-flex items-center gap-2 bg-blue-50 text-[#002B49] border border-blue-200 px-4 py-2 rounded-2xl text-xs font-extrabold shadow-2xs">
                    <Truck className="w-4 h-4 text-[#002B49] animate-pulse" />
                      ...
                      return (
                        <div key={step.id} className="text-center space-y-2 relative">
                          ...
                          <div className={`w-11 h-11 rounded-2xl mx-auto flex items-center justify-center transition-all duration-300 shadow-sm ${isCompleted
                              ...
                              : isCurrent
                                ? "bg-[#002B49] text-white ring-4 ring-[#002B49]/20 scale-110"
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4 text-xs">
              ...
              <div className="bg-slate-50 p-4 rounded-2xl border border-slate-200 space-y-2">
                <span className="text-slate-400 font-bold uppercase tracking-wider flex items-center gap-1.5 text-[11px]">
                  <User className="w-3.5 h-3.5 text-[#002B49]" /> Customer Info
                </span>
              ...
              <div className="bg-slate-50 p-4 rounded-2xl border border-slate-200 space-y-2">
                <span className="text-slate-400 font-bold uppercase tracking-wider flex items-center gap-1.5 text-[11px]">
                  <MapPin className="w-3.5 h-3.5 text-[#002B49]" /> Delivery Address
                </span>
              ...
              <div className="bg-slate-50 p-4 rounded-2xl border border-slate-200 space-y-2">
                <span className="text-slate-400 font-bold uppercase tracking-wider flex items-center gap-1.5 text-[11px]">
                  <CreditCard className="w-3.5 h-3.5 text-[#002B49]" /> Payment &amp; Courier
                </span>
                <div className="space-y-1">
                  <div className="flex items-center justify-between">
                    <span className="text-slate-500">Payment:</span>
                    <span className={`font-bold px-2 py-0.5 rounded-full text-[10px] ${trackedOrder.paymentStatus === "PAID"
            ...
            <div className="space-y-4 pt-2">
              <h3 className="font-black text-slate-900 text-sm flex items-center gap-2">
                <ShoppingBag className="w-4 h-4 text-[#002B49]" /> Ordered Items ({trackedOrder.items.length})
              </h3>
              ...
              <div className="border border-slate-200 rounded-2xl overflow-hidden divide-y divide-slate-100">
                {trackedOrder.items.map((item) => (
                  <div key={item.id} className="p-4 flex items-center gap-4 bg-white hover:bg-slate-50/50 transition">
                    ...
                    <div className="text-xs font-black text-[#002B49] shrink-0">
                ...
                <div className="p-4 bg-slate-50 space-y-1.5 text-xs">
                  ...
                  <div className="flex items-center justify-between text-sm font-black text-[#002B49] pt-2 border-t border-slate-200">
            ...
            <div className="bg-[#002B49]/5 border border-[#002B49]/20 rounded-2xl p-4 flex flex-col sm:flex-row items-center justify-between gap-4">
              ...
              <div className="flex items-center gap-3">
                <a
                  ...
                  className="bg-[#002B49] text-white font-bold text-xs px-4 py-2 rounded-full flex items-center gap-1.5 shadow-sm hover:bg-[#072450] transition shrink-0"
                >
                  <PhoneCall className="w-3.5 h-3.5" /> Call Hotline
                </a>
                <Link
                  href="/all-products"
                  className="border border-[#002B49] text-[#002B49] hover:bg-[#002B49] hover:text-white font-bold text-xs px-4 py-2 rounded-full transition shrink-0"
                >
                  Continue Shopping
                </Link>
              </div>
  ...
  return (
    <Suspense fallback={
      <div className="min-h-screen bg-slate-50 flex items-center justify-center p-8">
        <Loader2 className="w-8 h-8 animate-spin text-[#002B49]" />
      </div>
    }>
      <TrackOrderContent />
    </Suspense>
