...
export default function DeliveryInformationPage() {
  ...
  const deliverySteps = [
    ...
    {
      icon: <CreditCard className="w-8 h-8 text-[#122B5A]" />,
        ...
        <section className="bg-white rounded-3xl p-8 sm:p-12 shadow-sm border border-slate-200/80 space-y-6">
          ...
          <div className="rounded-2xl border border-slate-200 overflow-hidden shadow-xs">
            <table className="w-full text-left text-xs sm:text-sm">
              <thead className="bg-[#122B5A] text-white font-bold">
                <tr>
              ...
              <tbody className="divide-y divide-slate-100 text-slate-600 bg-white">
                {deliveryRates.map((rate, idx) => (
                  <tr key={idx} className="hover:bg-slate-50 transition">
                    ...
                    <td className="py-4.5 px-4 sm:px-6 text-right font-black text-lg text-[#122B5A]">{rate.rate}</td>
                  </tr>
                ))}
              </tbody>
            </table>
