...
export default function NotFound() {
  ...
  return (
    <div className="bg-slate-50 min-h-screen font-sans flex items-center justify-center py-16 px-4 relative overflow-hidden">
      ...
      <div className="absolute top-1/4 left-1/2 -translate-x-1/2 -translate-y-1/2 w-96 h-96 bg-gradient-to-tr from-[#002B49]/10 via-[#FF6600]/10 to-[#ff8c00]/10 rounded-full blur-3xl pointer-events-none" />
      ...
      <div className="max-w-2xl w-full bg-white/80 backdrop-blur-md rounded-3xl border border-slate-200/80 p-8 sm:p-14 text-center space-y-8 shadow-xl relative z-10">
        ...
        <div className="relative inline-block">
          <span className="text-7xl sm:text-9xl font-black italic tracking-tighter text-transparent bg-clip-text bg-gradient-to-r from-[#002B49] via-[#FF6600] to-[#e60000] drop-shadow-sm select-none">
            404
          </span>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
            {quickLinks.map((item, idx) => (
              <Link
                key={idx}
                href={item.href}
                className="bg-slate-50 hover:bg-[#002B49] hover:text-white text-slate-700 font-bold text-xs p-3 rounded-2xl border border-slate-100 transition-all duration-300 flex flex-col items-center gap-1.5 shadow-2xs group"
              >
        ...
        <div className="pt-4 flex flex-col sm:flex-row items-center justify-center gap-4">
          <Link
            href="/"
            className="w-full sm:w-auto inline-flex items-center justify-center gap-2 bg-[#002B49] hover:bg-[#FF6600] text-white font-bold text-xs sm:text-sm px-8 py-3.5 rounded-full shadow-lg hover:shadow-xl transition-all duration-300"
          >
